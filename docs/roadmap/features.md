...
> **W02** Monorepo Setup & Backend Scaffolding (done, merged), **W03** Angular 19 Frontend Scaffolding,
> **W04** GitHub Actions CI, **W05** Azure Infrastructure (Bicep), **W06** CD Pipelines, **W07** Local
> Env & Onboarding, **W08** Code Quality. See [`STATUS.md`](STATUS.md) for current progress.
...
- **Ingestion pipeline** (6 stages: Discoverer → Fetcher → Parser → Enricher → Persister → Indexer) for **CSJN** rulings and **SAIJ** legislation/jurisprudence + the **SAIJ thesaurus**. See tech docs 13–15. *(Tax sources are a new ingestion target — see F1.3.)*
...
| *(Viewer)* | Read-only access to specific projects (optional) |
...
> unit of work, hearings (*audiencias*), procedural deadline computation, the procedural agent,
...
*Features Roadmap — Legal Ai Ar (PwC Tax-Legal)*
