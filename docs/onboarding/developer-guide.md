...
**Cursor + Docker**) — is in the **[Developer Onboarding guide](README.md)**. Get the
...
├── docs/
│   ├── onboarding/                          # Local setup, developer guide, AI setup tutorial
│   ├── standards/                           # PwC internal app architecture (read first for new work)
│   ├── roadmap/                             # Work items by feature
│   ├── technical/                           # Product-specific technical docs
...
### Architecture standard (mandatory reference)

Before scaffolding or reviewing architecture, read **[`docs/standards/pwc-internal-app-architecture.md`](../standards/pwc-internal-app-architecture.md)** — the single guide for internal PwC apps (auth, layers, API, frontend, testing, delivery). Product docs in `docs/technical/` extend it; they must not contradict it. The [Definition of Done](../roadmap/DEFINITION-OF-DONE.md) requires syncing both when code changes.
...
| Document | What for |
|----------|----------|
| `docs/onboarding/appkit-npm-access.md` | JFrog login + `@appkit4` registry for the SPA |
| `docs/appkit4/README.md` | AppKit 4 components, tokens, patterns |
