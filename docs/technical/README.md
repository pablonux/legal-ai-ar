...
## Architecture baseline (read first)

All product-specific docs below extend the **[PwC Internal Application Architecture — Reference Guide](../standards/pwc-internal-app-architecture.md)**. That document is the single cross-product standard for auth, layering, API patterns, frontend structure, testing, and delivery. When implementation diverges from the guide, document the exception in the relevant product doc and get tech-lead approval.
...
| 10  | [System Architecture](./10-system-architecture.md)                         | ⚠️ _Imported, pending review._ System overview, ingestion design, data model, messaging, workers, infra, ADRs, risks                             |
| 11  | [Technical Specs](./11-technical-specs.md)                                 | ⚠️ _Imported, pending review._ Repository structure, Clean Architecture, CQRS, data model, API, frontend, testing, ADRs                          |
| 12  | [C4 Diagrams](./12-c4-diagrams.md)                                         | ⚠️ _Imported, pending review._ C4 context/container/component diagrams (sources in [`diagrams/`](./diagrams/))                                   |
...
| 14  | [CSJN Ruling Ingestion](./14-csjn-ruling-ingestion.md)                     | The CSJN _fallos_ ingestion pipeline as implemented: discovery sources, sjconsulta API, parsing, enrichment, indexing, config                    |
...
| 21  | [Business Layer — Projects & Workspaces](./21-business-workspace-model.md) | ⚠️ _Target model (R2–R3)._ Project/workspace business layer for PwC tax-legal: workspaces, documents, deliverables, internal KB, tax controversy |
| 22  | [API Endpoints](./22-api-endpoints.md)                                     | Minimal API groups, `LegalAiAr.Contracts` DTO layout, discovery and versioning policy                                                            |
| 23  | [Transactional Outbox & Domain Events](./23-outbox-domain-events.md)       | Outbox table, EF interceptor, in-process dispatcher, API `BackgroundService`; used by F2.1 aggregates (R2)                                       |
...
_Legal Ai Ar — Technical Documentation_
