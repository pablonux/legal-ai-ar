...
export interface ThinFeatureOptions {
  name: string;
  route: string;
  skipRoutes?: boolean;
}

export default function thinFeature(options: ThinFeatureOptions): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    context.logger.info(`Created thin feature "${dasherized}" at ${target}`);
